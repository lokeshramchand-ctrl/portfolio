...
export default function Hero() {
  ...
  useEffect(() => {
    ...
    gsap.to(cursor, { opacity: 0, duration: 0.55, repeat: -1, yoyo: true, ease: 'power1.inOut' });
    ...
    <section id="top" className="relative flex min-h-screen items-center overflow-hidden">
      ...
      >
        <p className="font-mono text-lg text-paper-400 md:text-2xl">
          <span ref={typedRef} />
          <span ref={cursorRef} className="ml-1 inline-block h-[1em] w-[0.5em] bg-amber-400 align-middle" />
        </p>
      </div>

      <div ref={heroRef} className="padding-x w-full pt-32 pb-20">
        <p className="font-mono text-sm text-amber-400">{siteConfig.locationCountry} — Available for work</p>
